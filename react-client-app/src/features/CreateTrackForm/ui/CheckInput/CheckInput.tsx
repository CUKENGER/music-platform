...
interface CheckInputProps {
    ...
    options: string[]
    setOptions: (e: string[] | ((e: string[]) => string[])) => void;
    value: string;
...
const CheckInput: FC<CheckInputProps> = memo(({ setValue, options, setOptions, value, currentOption}) => {
    ...
    const handleOptionClick = (option: string) => {
        if (selectedOption === option) {
            ...
            if (selectedOption) {
                setOptions((prevOptions: string[]) => [...prevOptions, selectedOption]);
                setValue(selectedOption)
            }
            setOptions((prevOptions: string[]) => prevOptions.filter((item) => item !== option));
